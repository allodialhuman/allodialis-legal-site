...
export default function TestimonialCarousel({ lang, testimonials }: TestimonialSectionProps) {
  const isEn = lang === 'en'
  const reviewCount = testimonials.length
  const slides = useMemo(() => [testimonials[reviewCount - 1], ...testimonials, testimonials[0]], [reviewCount, testimonials])
  ...
  useEffect(() => {
    if (window.location.hash !== '#velemenyek') return

    const firstFrame = window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => {
        document.getElementById('velemenyek')?.scrollIntoView({ block: 'start' })
      })
    })
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (!entry.isIntersecting) setIsAnimating(false)
      },
  ...
  const autoplayPaused =
    userPaused || !isInViewport || isHovered || hasFocus || isExpanded || isDocumentHidden || prefersReducedMotion
  ...
  return (
    <section ref={sectionRef} className="testimonial-carousel-live py-24 md:py-32 section-obsidian relative overflow-hidden border-b border-gold/10">
                      ...
                      <div id={`testimonial-content-${slidePosition}`} className="flex-1 flex flex-col justify-center">
                        {logicalIndex === 0 ? (
                          <div className="space-y-6">
                            ...
                            <p className="text-cream/90 text-sm md:text-lg leading-relaxed font-normal font-sans">{paragraphs[1]}</p>
                            <p className="text-cream/75 text-sm md:text-base leading-relaxed font-normal font-sans">{paragraphs[2]}</p>
                          </div>
                        ) : showFullText ? (
                          <div className="space-y-4 text-left sm:text-center">
                            {paragraphs.map((paragraph, paragraphIndex) => (
                              <p key={paragraphIndex} className="text-cream/85 text-sm md:text-base leading-relaxed font-sans font-normal">
                                {paragraph}
                              </p>
                            ))}
                          </div>
                        ) : (
                          <p className="testimonial-excerpt font-sans text-lg md:text-2xl font-normal text-cream/90 leading-relaxed">
                            „{fullText}”
                          </p>
                        )}
                      </div>
